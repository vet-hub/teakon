...
export default function CriteriaSelection({ chosenCriterias, setChosenCriterias, allTeearten, allAnbaugebiete, allBenefits, allAromen, allCoffein }) {
    ...
    return (
        <div className="criteriaSelection flex flex-col items-center w-full p-5">
            <h2 className=" mb-2 lg:mb-10 font-bold md:m-5 max-[350px]:text-2xl  min-[351px]:text-3xl md:text-4xl text-black dark:text-gray-300" >Kriterien Auswahl</h2>
            <div className="mainCriterias text-center md:flex md:justify-evenly md:flex-wrap w-full md:gap-2 ">
                ...
                <div className="min-h-8 mt-3 md:criteriaSelector md:opacity-80 shadow border visible border-black rounded-xl md:flex md:justify-center md:w-1/6 md:mb-5 md:aspect-square md:hover:cursor-pointer bg-flavour-bg md:bg-cover" onClick={setToFlavour}>
                    <h3 className="italic criteriaName my-1 font-bold min-[1280px]:text-3xl self-center text-gray-800  ">Geschmack</h3>
                </div>
            ...
            {chosenCriterias.length > 0 && <><p className="text-black dark:text-gray-300 mt-5">Ausgewählte Kriterien:</p>
                <SelectedCriterias chosenCriterias={chosenCriterias} setChosenCriterias={setChosenCriterias} /></>}
            <button onClick={startSearch} className="bg-black text-white hover:bg-white dark:text-gray-300 font-bold hover:text-black py-3 px-7 mt-7 mb-7 border border-black hover:border-black rounded-full duration-200">Ergebnisse anzeigen</button>
        </div>
    )
}